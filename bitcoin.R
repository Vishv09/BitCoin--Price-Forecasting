...
#Performing linear regression on trend
lreg1 <- tslm(training_data ~ trend)
tslm_fit1=forecast(lreg1, h=12)
summary(tslm_fit1)
plot(tslm_fit1, ylab="Bitcoin price",
     xlab="t")

#Performing linear regression on Trend and season
lreg2 <- tslm(training_data ~ trend + season)
tslm_fit2=forecast(lreg2, h=12)
summary(tslm_fit2)
plot(tslm_fit2, ylab="Bitcoin price",
     xlab="t")
...
legend("topleft",lty=1,col="orange",cex=0.6,legend=c("3-MA"))
plot(training_data, main="Bitcoin price",
     ylab="$ million", xlab="Year")
...
legend("topleft",lty=1,col="green",cex=0.6,legend=c("9-MA"))
...
acc.linear=accuracy(tslm_fit1,testing_data)
acc.linear_season=accuracy(tslm_fit2,testing_data)
...
acc.table<-rbind(acc.mean, acc.naive, acc.snaive, acc.linear, acc.linear_season, acc.ses, acc.holt, acc.add, acc.multi, acc.arima)
acc.table
row.names(acc.table)<-c('Mean training','Mean test', 'Naive training', 'Naive test', 'Seasonal. Naive training', 'Seasonal. Naive test' ,'Linear training', 'Linear test','season-trend training', 'season-trend test',"ses training", "ses test","Holt's Linear training", "Holt's Linear test", 'Add training', 'Add test','Multi training', 'Multi test','ARIMA training', 'ARIMA test')
